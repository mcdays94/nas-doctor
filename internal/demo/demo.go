	...
	return []internal.RemoteServerStatus{
		{
			...
			Platform: "synology", Version: "0.7.0", Uptime: "142d 8h", OverallHealth: "healthy",
			CriticalCount: 0, WarningCount: 1, InfoCount: 3,
			LastPoll: time.Now().Add(-30 * time.Second).Format(time.RFC3339),
			Summary: &internal.FleetServerSummary{
				CPUUsage: 12.4, MemPercent: 45.2, MemTotalMB: 16384, MemUsedMB: 7405,
				CPUModel: "Intel Celeron J4125", CPUCores: 4, LoadAvg1: 0.82, IOWait: 3.1,
				DriveCount: 4, DrivesHealthy: 3, DrivesWarning: 1, DrivesCritical: 0,
				TotalStorageTB:  32.0,
				DockerAvailable: true, ContainersRunning: 8, ContainersTotal: 10,
				ServiceChecksUp: 3, ServiceChecksDown: 0, ServiceChecksTotal: 3,
				Findings: []internal.FleetFinding{
					{Severity: "warning", Title: "Drive /dev/sdc has 3 reallocated sectors", Category: "SMART"},
					{Severity: "info", Title: "Synology DSM update 7.2.2 available", Category: "System"},
					{Severity: "info", Title: "Container 'watchtower' has pending image update", Category: "Docker"},
					{Severity: "info", Title: "Volume /volume1 at 72% capacity", Category: "Storage"},
				},
			},
		},
		{
			Server: servers[1], Online: true, Hostname: "plex-tower",
			Platform: "unraid", Version: "0.7.0", Uptime: "30d 14h", OverallHealth: "warning",
			CriticalCount: 0, WarningCount: 4, InfoCount: 2,
			LastPoll: time.Now().Add(-25 * time.Second).Format(time.RFC3339),
			Summary: &internal.FleetServerSummary{
				CPUUsage: 67.3, MemPercent: 82.1, MemTotalMB: 65536, MemUsedMB: 53821,
				CPUModel: "AMD Ryzen 7 5800X", CPUCores: 16, LoadAvg1: 4.21, IOWait: 22.4,
				DriveCount: 8, DrivesHealthy: 6, DrivesWarning: 2, DrivesCritical: 0,
				TotalStorageTB:  96.0,
				DockerAvailable: true, ContainersRunning: 24, ContainersTotal: 26,
				ServiceChecksUp: 5, ServiceChecksDown: 1, ServiceChecksTotal: 6,
				Findings: []internal.FleetFinding{
					{Severity: "warning", Title: "CPU usage above 60% for 2 hours", Category: "System"},
					{Severity: "warning", Title: "Memory usage at 82% (53.8 GB / 64 GB)", Category: "System"},
					{Severity: "warning", Title: "Drive /dev/sde has 8 reallocated sectors", Category: "SMART"},
					{Severity: "warning", Title: "I/O wait at 22% — possible disk bottleneck", Category: "System"},
					{Severity: "info", Title: "Parity check completed — 0 errors", Category: "Parity"},
					{Severity: "info", Title: "Container 'tdarr' using 12% CPU", Category: "Docker"},
				},
			},
		},
		{
			Server: servers[2], Online: true, Hostname: "pve-node1",
			Platform: "linux", Version: "0.6.2", Uptime: "89d 2h", OverallHealth: "healthy",
			CriticalCount: 0, WarningCount: 0, InfoCount: 1,
			LastPoll: time.Now().Add(-20 * time.Second).Format(time.RFC3339),
			Summary: &internal.FleetServerSummary{
				CPUUsage: 8.7, MemPercent: 31.4, MemTotalMB: 131072, MemUsedMB: 41165,
				CPUModel: "Intel Xeon E-2288G", CPUCores: 16, LoadAvg1: 0.43, IOWait: 1.2,
				DriveCount: 2, DrivesHealthy: 2, DrivesWarning: 0, DrivesCritical: 0,
				TotalStorageTB:  3.84,
				DockerAvailable: false, ContainersRunning: 0, ContainersTotal: 0,
				ServiceChecksUp: 2, ServiceChecksDown: 0, ServiceChecksTotal: 2,
				Findings: []internal.FleetFinding{
					{Severity: "info", Title: "2 VMs stopped (dev-test, staging)", Category: "Proxmox"},
				},
			},
		},
		{
...
func demoSystem() internal.SystemInfo {
	return internal.SystemInfo{
		...
		TopProcesses: []internal.ProcessInfo{
			...
			{PID: 3156, User: "nobody", CPU: 5.4, Mem: 2.8, Command: "python3 /app/tdarr/Tdarr_Node", ContainerName: "tdarr", ContainerID: "c3d4e5f6a1b2"},
			{PID: 4201, User: "root", CPU: 3.2, Mem: 1.5, Command: "/usr/sbin/shfs"},
			{PID: 5678, User: "root", CPU: 2.1, Mem: 3.4, Command: "mongod --config /etc/mongod.conf", ContainerName: "unifi", ContainerID: "d4e5f6a1b2c3"},
			{PID: 6789, User: "root", CPU: 1.8, Mem: 0.9, Command: "nginx: master process", ContainerName: "swag", ContainerID: "e5f6a1b2c3d4"},
			{PID: 7890, User: "nobody", CPU: 1.2, Mem: 0.4, Command: "/usr/bin/wireguard-go wg0"},
			{PID: 8901, User: "root", CPU: 0.8, Mem: 2.1, Command: "postgres: writer process", ContainerName: "home-assistant", ContainerID: "f6a1b2c3d4e5"},
