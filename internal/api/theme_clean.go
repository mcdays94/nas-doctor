...
(function() {
  ...
  function renderDashboard(status, snapshot) {
    ...
    if (snapshot && snapshot.network && snapshot.network.interfaces && snapshot.network.interfaces.length > 0) {
      ...
      html += '<tr style="color:#808080;font-size:11px"><th style="text-align:left;padding:6px 0;border-bottom:1px solid rgba(0,0,0,0.08)">Interface</th><th style="text-align:left;padding:6px 0;border-bottom:1px solid rgba(0,0,0,0.08)">State</th><th style="text-align:left;padding:6px 0;border-bottom:1px solid rgba(0,0,0,0.08)">Speed</th><th style="text-align:left;padding:6px 0;border-bottom:1px solid rgba(0,0,0,0.08)">IP</th></tr>';
      for (var ni = 0; ni < snapshot.network.interfaces.length; ni++) {
        var iface = snapshot.network.interfaces[ni];
        html += '<tr><td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04)">' + escapeHTML(iface.name) + '</td>';
        html += '<td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04);color:' + (iface.state === "UP" ? "#16a34a" : "#d97706") + '">' + escapeHTML(iface.state) + '</td>';
        html += '<td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04)">' + escapeHTML(iface.speed || "—") + '</td>';
        html += '<td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04)">' + escapeHTML(iface.ipv4 || "—") + '</td></tr>';
      }
      html += '</table></div>';
    }
    ...
    if (snapshot && snapshot.parity) {
      ...
      if (par.history && par.history.length > 0) {
        ...
        for (var pi = 0; pi < par.history.length; pi++) {
          ...
          html += '<tr><td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04)">' + escapeHTML(pc.date) + '</td>';
          html += '<td style="padding:5px 0;border-bottom:1px solid rgba(0,0,0,0.04)">' + escapeHTML(pc.duration_human || "—") + '</td>';
